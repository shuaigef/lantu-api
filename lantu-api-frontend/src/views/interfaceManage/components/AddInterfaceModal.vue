<template>
  <a-modal v-model:visible="props.visible" @ok="handleOk" @cancel="handleCancel" :width="700">
    <template #title>
      新建接口信息
    </template>
...
import {addInterfaceInfo} from "../../../api/interfaceInfo";
...
const formData = ref<API.InterfaceInfoAddParams>({
  name: "",
  description: "",
  interfaceAvatar: "",
  consumePoints: 0,
...
const handleOk = async () => {
  try {
    const res = await addInterfaceInfo(formData.value)
    if (res.code == 0) {
      // 子组件触发父组件事件
      handleCancel()
